...
function log(message: string, data?: Record<string, unknown>) {
  ...
  console.log(`[${timestamp}] [CRON-MAINTENANCE] ${message}${dataStr}`);
}

/**
 * GET /api/cron/job-maintenance
 *
 * Automated job maintenance that runs on a schedule (Vercel Cron).
 ...
 * 4. Escalates jobs with repeated failures to needs_review for human attention
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Find failed jobs whose video is now cached (another job succeeded)
    // ─────────────────────────────────────────────────────
    const { data: failedJobs } = await supabase
      .from("filter_jobs")
      .select("id, job_id, youtube_id, auto_retry_count, needs_review, last_auto_retry_at")
      .eq("status", "failed");

    if (failedJobs && failedJobs.length > 0) {
      const failedYoutubeIds = [...new Set(failedJobs.map((j) => j.youtube_id))];

      // Check which videos are now cached
      ...
      // Delete resolved failed jobs (they're just noise now)
      const toResolve = failedJobs.filter((j) => resolvedIds.has(j.youtube_id));

      if (toResolve.length > 0) {
        const { error: resolveError } = await supabase
          .from("filter_jobs")
          .delete()
          .in("id", toResolve.map((j) => j.id));
      ...
      // ─────────────────────────────────────────────────────
      // Step 3 & 4: Auto-retry unresolved failed jobs or escalate
      // ─────────────────────────────────────────────────────
      ...
      for (const [youtubeId, videoJobs] of jobsByVideo) {
        // Pick the most recent job (lowest auto_retry_count or most recent)
        ...
        // Check if this job has exceeded max retries → escalate
        if (retryCount >= MAX_AUTO_RETRIES) {
          // Mark all jobs for this video as needs_review
          const { error: escalateError } = await supabase
            .from("filter_jobs")
            .update({ needs_review: true })
            .in("id", videoJobs.map((j) => j.id));

          if (escalateError) {
            results.errors.push(`escalate ${youtubeId}: ${escalateError.message}`);
          } else {
        ...
        // Check cooldown — don't retry too quickly
        ...
        // Attempt auto-retry
        try {
          // Clear cached video data so orchestrator reprocesses
          try {
          ...
          if (!orchResponse.ok) {
            // Orchestrator rejected — mark failed again with incremented retry count
            await supabase
              .from("filter_jobs")
              .update({
                status: "failed",
          ...
          // Mark sibling failed jobs for this video as resolved (delete them)
          ...
          // Mark job as failed with retry count incremented
          await supabase
            .from("filter_jobs")
            .update({
              status: "failed",
    ...
    log("=== Job Maintenance Complete ===", results);
    ...
    log("EXCEPTION", { error: errorMsg });
    console.error("Cron job-maintenance error:", error);
    return NextResponse.json(
